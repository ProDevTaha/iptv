...
<nav class="navbar navbar-expand-lg navbar-light bg-dark"  >
    <div class="container-fluid">
      ...
      <div class="collapse navbar-collapse" id="navbarSupportedContent">
        <ul class="navbar-nav me-auto mb-2 mb-lg-0">
          ...
          <li class="nav-item">
            <a class="nav-link text-light fw-bold" href="/Order_Trial">Order Trial <span class="badge badge-danger ms-1" style="background: rgb(244, 7, 7)">{{$countTrial}}</span></a>
          </li> 
          <li class="nav-item">
  ...
  <div class="d-flex fex-wrap container justify-content-between shadow p-3 mb-5 mt-5 rounded">
    ...
    <form action="{{route('Order_Trial')}}" method="get" class="d-flex flex-wrap align-items-center w-50">
        <span >from</span>
    ...
    <table class="table table-striped table-hover shadow container  p-3 mb-5  text-dark rounded">
        ...
        <tbody>
            @foreach ($orders as $order) 
                <tr>
                    <th scope="col">{{$order->firstname}}</th>
                    <th scope="col">{{$order->lastname}}</th>
