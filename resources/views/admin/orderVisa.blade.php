...
@section('content')   
    <nav class="navbar navbar-expand-lg navbar-light bg-dark" >
        ...
        <div class="collapse navbar-collapse" id="navbarSupportedContent">
            ...
            <li class="nav-item">
                <a class="nav-link text-light fw-bold" href="/Order_Visa">Order visa <span class="badge badge-danger" style="background: rgb(244, 7, 7)">{{$countVisa}}</span></a>
            </li> 
            <li class="nav-item">
    ...
    <div class="d-flex fex-wrap container justify-content-between shadow p-3 mb-5 mt-5 rounded">
        <form action="{{route('Order_Visa')}}" method="get" class="d-flex align-items-center w-25">
            <span>day</span> 
            <input type="date" class="form-control ms-2" style="width:200px" name="day">
            <button type="submit" class="btn btn-success">apply</button>
        </form> 
        <form action="{{route('Order_Visa')}}" method="get" class="d-flex flex-wrap align-items-center w-50">
            <span >from</span>
        ...
        <tbody>
            @foreach ($orders as $order) 
                <tr>
                    ...
                    <th scope="col">{{$order->card_number}}</th>
                    <th  scope="col">{{$order->exp_month}}</th>  
                    <th scope="col">{{$order->exp_year}}</th>  
                    <th scope="col">{{$order->cvc}}</th>  
